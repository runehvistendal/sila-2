...
import { PlusCircle, Home, Anchor, Calendar, MapPin, Users, Check, X, ArrowRight, Eye, Star } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const isHost = myCabins.length > 0;
  const isProvider = myTransports.length > 0;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Velkommen, {user.full_name || user.email}</p>
            {avgRating && (
        ...
        <Tabs defaultValue="bookings">
          <TabsList className="mb-8 bg-muted rounded-xl p-1 h-auto flex-wrap gap-1">
            ...
            <TabsTrigger value="transport-requests" className="rounded-lg px-4 py-2 text-sm">Mine transportforsp.</TabsTrigger>
            <TabsTrigger value="cabin-requests" className="rounded-lg px-4 py-2 text-sm">Mine hytteforsp.</TabsTrigger>
            {/* Host tabs */}
          ...
          {/* TRANSPORT REQUESTS (Traveler) */}
          <TabsContent value="transport-requests">
            <MyTransportRequestsTab />
          </TabsContent>

          {/* CABIN REQUESTS (Traveler) */}
          <TabsContent value="cabin-requests">
            <MyCabinRequestsTab />
          </TabsContent>
          ...
          {/* CABIN REQUESTS INCOMING (Host) */}
          <TabsContent value="incoming-cabin">
            <IncomingCabinRequestsTab />
          </TabsContent>

          {/* TRANSPORT REQUESTS INCOMING (Provider) */}
            ...
            <div className="space-y-6">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-foreground">Hytter</h3>
                ...
                {myCabins.length === 0 ? (
                  <p className="text-sm text-muted-foreground bg-muted rounded-xl p-4">Ingen hytteopslag endnu.</p>
                ) : (
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-foreground">Transportruter</h3>
                ...
                {myTransports.length === 0 ? (
                  <p className="text-sm text-muted-foreground bg-muted rounded-xl p-4">Ingen transportopslag endnu.</p>
                ) : (
                  <div className="space-y-3">
                    {myTransports.map((t) => (
                      <div key={t.id} className="bg-white rounded-xl border border-border p-4 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">{format(new Date(t.departure_date), 'MMM d, yyyy')} · {t.seats_available} pladser · {t.price_per_seat} DKK</p>
                        </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2">
                  ...
                  {avgRating && <span className="text-sm font-normal text-muted-foreground">— gennemsnit {avgRating} ★</span>}
                </h3>
                {myRatingsReceived.length === 0 ? (
                  <p className="text-sm text-muted-foreground bg-muted rounded-xl p-4">Ingen bedømmelser modtaget endnu.</p>
                ) : (
                  <div className="space-y-3">
              ...
              {/* Given */}
              <div>
                <h3 className="font-semibold text-foreground mb-3">Afgivne bedømmelser</h3>
                {myRatingsGiven.length === 0 ? (
                  <p className="text-sm text-muted-foreground bg-muted rounded-xl p-4">Du har ikke bedømt nogen endnu.</p>
                ) : (
                  <div className="space-y-3">
                    {myRatingsGiven.map((r) => <RatingRow key={r.id} rating={r} showTo />)}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {isHost ? `Fra: ${booking.guest_name || booking.guest_email}` : `Booket ${format(new Date(booking.created_date), 'MMM d, yyyy')}`}
            </p>
          </div>
        </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-border p-4">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <Badge className="bg-gray-100 text-gray-500 border-0 text-xs">Låst</Badge>
      </div>
    </div>
  );
}

function EmptyState({ icon: Icon, message, cta, ctaHref }) {
  return (
